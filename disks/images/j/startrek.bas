  ...
  211 REM ***
  212 REM *** CONVERTED TO BBC BASIC FOR AGON LIGHT 26.MAR.2024 BY J. ROSE
  213 REM *** PRESERVED LINE NUMBERS AS MUCH AS POSSIBLE, BUT THEY ARE ALREADY
  214 REM *** CHANGED FROM JOHN BORDERS. UNWOUND UNLIKE MULTIPLE STATEMENTS
  215 REM *** ESPECIALLY FOR LOOPS AND GOSUBS AND ON GOTO'S FOR BBC BASIC
  219 REM ***
  ...
  370 T=INT(RND(1)*20+20)*100: T0=T
  ...
  490 Q1=FNR(1):Q2=FNR(1)
  491 S1=FNR(1):S2=FNR(1)
  530 FOR I = 1 TO 9
  531   C(I,1)=0
  532   C(I,2)=0
  533 NEXT I
 ...
 1500 PRINT
 1501 K3=INT(G(Q1,Q2)*.01):B3=INT(G(Q1,Q2)*.1)-10*K3
 ...
 1681 Z1=S1:Z2=S2
 1682 GOSUB 8670
 1683 IF K3<1 THEN 1820
 1720 FOR I=1 TO K3
 1721   GOSUB 8590
 1722   A$="+K+"
 1779   Z1=R1:Z2=R2
 1780   GOSUB 8670
 1781   K(I,1)=R1:K(I,2)=R2:K(I,3)=S9*(0.5+RND(1))
 1782 NEXT I
 1820 IF B3<1 THEN 1910
 1880 GOSUB 8590
 1881 A$=">!<"
 1882 Z1=R1:B4=R1:Z2=R2:B5=R2
 1883 GOSUB 8670
 1910 FOR I=1 TO S3
 1911   GOSUB 8590
 1912   A$=" * "
 ...
 2610   A$="   "
 2611   Z1=K(I,1):Z2=K(I,2)
 2612   GOSUB 8670
 2613   GOSUB 8590
 2660   K(I,1)=Z1:K(I,2)=Z2
 2661   A$="+K+"
 2662   GOSUB 8670
 2700 NEXT I
 2701 GOSUB 6000
 ...
 3501 Q1=INT(X/8):Q2=INT(Y/8)
 3550 S1=INT(X-Q1*8):S2=INT(Y-Q2*8)
 ...
 4061   N(1)=-1:N(2)=-2:N(3)=-3
 4062   FOR J=Q2-1 TO Q2+1
 ...
 4700 IF P<=0 THEN PRINT"ALL PHOTON TORPEDOS EXPENDED":GOTO 1990
 4730 IF D(5)<0 THEN PRINT"PHOTON TUBES ARE NOT OPERATIONAL":GOTO 1990
 4760 INPUT"PHOTON TORPEDO COURSE (1-8)";C1
 ...
 5061 Z1=X:Z2=Y
 5062 GOSUB 8830
 5063 IF Z3=0 THEN 5210
 5110 PRINT"*** KLINGON DESTROYED ***"
 5111 K3=K3-1:K9=K9-1
 5112 IF K9<=0 THEN 6370
 5150 FOR I=1 TO 3
 ...
 5611 GOTO 1990
 5630 E=E+S-X:S=X
 5631 PRINT"DEFLECTOR CONTROL ROOM REPORT:"
 ...
 6100   PRINT"      <SHEILDS DOWN TO ";S;" UNITS>"
 6101   IF H<20 THEN 6200
 6120   IF RND(1)>.6 OR H/S<=.02 THEN 6200
 6140   R1=FNR(1):D(R1)=D(R1)-H/S-.5*RND(1)
 6141   GOSUB 8790
 ...
 6290 PRINT:PRINT
 6291 IF B9=0 THEN 6360
 6310 PRINT"THE FEDERATION IS IN NEED OF A NEW STARSHIP COMMANDER"
 6320 PRINT"FOR A SIMILAR MISSION -- IF THERE IS A VOLUNTEER,"
 6330 INPUT"LET THEM STEP FORWARD AND ENTER 'AYE'";A$
 6331 IF a4="AYE" THEN RUN: REM WAS GOTO 10 BUT BBC BASIC CAN'T RE-DIM LINE 330
 6360 END
 ...
 6730 PRINT:PRINT"*** SHORT RANGE SENSORS ARE OUT ***":PRINT
 6731 RETURN
 6770 O1$="---------------------------------"
 6771 PRINT O1$
 6772 FOR I=1 TO 8
 ...
 7740   Z4=I:Z5=1
 7741   GOSUB 9030
 7742   J0=INT(15-.5*LEN(G2$)):PRINT TAB(J0);G2$;
 7800   Z5=5
 7801   GOSUB 9030
 7802   J0=INT(39-.5*LEN(G2$)):PRINT TAB(J0);G2$;
 7850   PRINT:PRINT O1$
 7851 NEXT I
 7852 PRINT
 7853 GOTO 1990
 ...
 8201   REM CALCULATE THE DISTANCE, TEST IF ZERO
 8202   D2=SQR(X^2+A^2)
 8203   IF D2=0 THEN 8460
 8220   X=X-A:A=C1-W1
 8221   IF X<0 THEN 8350
 8250   IF A<0 THEN 8410
 8260   IF X>0 THEN 8280
 8270   IF A=0 THEN C1=5:GOTO 8290
 8280   C1=1
 8290   IF ABS(A)<=ABS(X) THEN 8330
 8310   IF A=0 THEN D=C1 ELSE D=C1+(((ABS(A)-ABS(X))+ABS(A))/ABS(A))
 8311   PRINT"DIRECTION = ";D
 8312   GOTO 8460
 8330   IF X=0 THEN D=C1 ELSE D=C1+(ABS(A)/ABS(X))
 8331   PRINT"DIRECTION = ";D
 8332   GOTO 8460
 8350   IF A>0 THEN C1=3:GOTO 8420
 8360   IF X<>0 THEN C1=5:GOTO 8290
 8410   C1=7
 8420   IF ABS(A)>=ABS(X) THEN 8450
 8430   IF X=0 THEN D=C1 ELSE D=C1+(((ABS(X)-ABS(A))+ABS(X))/ABS(X))
 8431   PRINT"DIRECTION = ";D
 8432   GOTO 8460
 8450   IF A=0 THEN D=C1 ELSE D=C1+(ABS(X)/ABS(A))
 8451   PRINT"DIRECTION = ";D
 8460   PRINT"DISTANCE = ";D2
 8461   IF H8=1 THEN 1990:REM JUMP OUT OF THE I LOOP IF CALLED FROM 7352
 8480 NEXT I
 8481 GOTO 1990
 8490 REM STARBASE NAVIGATION DATA
 ...
 8590 R1=FNR(1):R2=FNR(1)
 8591 A$="   "
 8592 Z1=R1:Z2=R2
 8593 GOSUB 8830
 8594 IF Z3=0 THEN 8590
 8600 RETURN
